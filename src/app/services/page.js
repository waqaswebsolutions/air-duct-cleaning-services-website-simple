        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {services.map((service, index) => (
            <div key={index} className="p-6 text-center transition bg-white border-t-4 rounded-lg shadow-md hover:shadow-lg border-secondary">
              ...
              <p className="mb-4 font-bold text-accent">Starting at {service.price}</p>
              <Link href="/contact">
                <button className="px-4 py-2 text-sm font-semibold text-white transition-all rounded-lg shadow-md cursor-pointer bg-secondary hover:scale-105">
                  Get Estimate
                </button>
              </Link>
            </div>
